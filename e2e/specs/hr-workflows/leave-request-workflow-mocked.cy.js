/**
 * Leave Request Workflow - Mocked Version
 * Tests leave request functionality using mocking to avoid server dependencies
 */
...
describe('Leave Request Workflow - Mocked', () => {
    beforeEach(() => {
        setupMocking();
        cy.task('log', 'Setting up mocked leave request tests...');
    });

    afterEach(() => {
        cy.task('log', 'Cleaning up mocked leave request test data...');
    });

    describe('Basic Functionality', () => {
        it('should perform basic leave request operations', () => {
            cy.task('log', 'Testing basic leave request functionality...');
            
            const result = mockSuccess('leave request operation completed', {
                operation: 'leave request',
                status: 'completed',
                timestamp: new Date().toISOString()
            });

            cy.task('log', `✅ leave request operation successful: ${result.message}`);
            
            expect(result.success).to.be.true;
            expect(result.data.status).to.equal('completed');
        });

        it('should handle leave request validation', () => {
            cy.task('log', 'Testing leave request validation...');
        ...
        it('should handle leave request errors gracefully', () => {
            cy.task('log', 'Testing leave request error handling...');
    ...
    describe('Advanced Features', () => {
        it('should support advanced leave request features', () => {
            cy.task('log', 'Testing advanced leave request features...');
            
            const advancedResult = mockApiOperation('create', 'leave request', {
                feature: 'advanced',
                enabled: true,
        ...
        it('should integrate with other systems', () => {
            cy.task('log', 'Testing leave request system integration...');
    ...
    describe('Performance and Security', () => {
        it('should meet performance requirements', () => {
            cy.task('log', 'Testing leave request performance...');
        ...
        it('should enforce security measures', () => {
            cy.task('log', 'Testing leave request security...');
