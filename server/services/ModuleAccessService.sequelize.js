...
import ModuleManagementService from '../platform/services/ModuleManagementService.sequelize.js';
...
/**
 * Module Access Service (Sequelize)
...
class ModuleAccessService {
  ...
  async _checkAccessFallback(companySlug, moduleKey, cacheKey) {
    ...
    let company = await Company.findOne({ where: { slug: companySlug } });
    ...
    if (!company && companySlug.includes('-')) {
      ...
      company = await Company.findOne({ where: { slug: underscoreSlug } });
    }
    ...
    if (!company && companySlug.includes('_')) {
      ...
      company = await Company.findOne({ where: { slug: hyphenSlug } });
    }
    ...
    const result = await ModuleManagementService.checkModuleAccess(company.id, moduleKey);
  ...
  async _getCompanyModulesFallback(companySlug, cacheKey) {
    ...
    const result = await ModuleManagementService.getCompanyModules(company.id);
  ...
  async updateUsage(companySlug, usage) {
    try {
      ...
      const result = await ModuleManagementService.updateCompanyUsage(company.id, usage);
