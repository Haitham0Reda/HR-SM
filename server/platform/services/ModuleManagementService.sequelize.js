...
/**
 * Module Management Service (Sequelize)
...
class ModuleManagementService {
   ...
   * @param {string} companyId - Company ID (UUID)
  ...
  async generateCompanyLicense(companyId, secretKey) {
    try {
      ...
      for (const [key, config] of Object.entries(companyModules)) {
        if (config.enabled) {
          modules[key] = {
            enabled: true,
            tier: config.tier,
      ...
      const now = new Date();
      const endDate = company.subscription?.endDate ? new Date(company.subscription.endDate) : new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);
      const validityDays = Math.ceil((endDate - now) / (1000 * 60 * 60 * 24));
      ...
      const licenseData = generateLicenseFile({
        ...
        metadata: {
          ...
          notes: `${company.subscription?.plan || 'trial'} license`
        }
      }, secretKey);
  ...
  async updateCompanyUsage(companyId, usage) {
    try {
      ...
      if (usage.employees !== undefined) currentUsage.employees = usage.employees;
      if (usage.storage !== undefined) currentUsage.storage = usage.storage;
      if (usage.apiCalls !== undefined) currentUsage.apiCalls = usage.apiCalls;
      
      currentUsage.lastUpdated = new Date();
      company.usage = currentUsage;
      company.changed('usage', true);
      await company.save();
   ...
   * @param {Array} companyIds - Array of company IDs (UUIDs)
   * @param {string} moduleKey - Module key
   * @param {string} tier - Pricing tier
