...
const PayrollPage = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 120,
            renderCell: (params) => {
                const [year, month] = params.row.period.split('-');
                const monthName = months[parseInt(month) - 1];
                return `${monthName} ${year}`;
            }
        },
        {
            field: 'deductionsCount',
            headerName: 'Deductions',
            width: 120,
            renderCell: (params) => `${params.row.deductions?.length || 0} items`
        },
        {
            field: 'totalDeductions',
            headerName: 'Total Deductions',
            width: 140,
            renderCell: (params) => `$${params.row.totalDeductions?.toFixed(2) || '0.00'}`
        },
        {
            field: 'netSalary',
            ...
            width: 120,
            renderCell: (params) => {
                const role = params.row.employee?.role;
                const baseSalary = getBaseSalaryByRole(role);
                const netSalary = baseSalary - (params.row.totalDeductions || 0);
                return (
