...
const UsersPage = () => {
    ...
    const handleBulkUpload = async () => {
        ...
        try {
            ...
            // Count auto-generated emails
            const autoGeneratedCount = response.results?.filter(result => result.emailGenerated).length || 0;
            
            if (response.created > 0) {
                let message = `Successfully created ${response.created} user(s)`;
                
                if (autoGeneratedCount > 0) {
                    message += ` (${autoGeneratedCount} with auto-generated emails)`;
                }
            ...
            if (response.errors && response.errors.length > 0) {
                console.log('Bulk upload errors:', response.errors);
                
                // Show first few errors as examples
                const errorSample = response.errors.slice(0, 3);
                const errorMessage = errorSample.map(err => 
                    `Row ${err.row}: ${err.error}`
                ).join('; ');
                
                showNotification(`Errors: ${errorMessage}${response.errors.length > 3 ? '...' : ''}`, 'error');
            }
    ...
    const handleBulkDownloadPhotos = async () => {
        try {
            ...
            // First, validate the token by checking a simple endpoint
            console.log('Validating authentication...');
            const authTestResponse = await fetch(`${apiBaseUrl}/users?limit=1`, {
            ...
            if (!authTestResponse.ok) {
                if (authTestResponse.status === 401) {
                    ...
                    throw new Error(`Authentication error (${authTestResponse.status}) - please try again`);
                }
            }

            // If authentication is valid, proceed with download
            const downloadUrl = `${apiBaseUrl}/users/bulk-download-photos?token=${encodeURIComponent(token)}&userIds=${encodeURIComponent(JSON.stringify(userIds))}`;
            
            console.log('Opening download URL:', downloadUrl.substring(0, 100) + '...');
                ...
                <DialogContent sx={{ pt: 3 }}>
                    ...
                    <Box sx={{
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.85rem', ml: 3, mb: 0.5 }}>
                            • <strong>username</strong> - Unique username (used for email generation)
                        </Typography>
                        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.85rem', ml: 3, mb: 0.5 }}>
                            • <strong>email</strong> - Email address (optional - will be auto-generated from username if not provided)
                        </Typography>
                    ...
                    >
                        ...
                        <Typography variant="body2">
                            If you leave the <strong>email</strong> column empty, emails will be automatically generated using the format: <code>firstname.lastname@company-domain.com</code>
                        </Typography>
                        <Typography variant="body2" sx={{ mt: 0.5 }}>
                            If a name combination already exists, a number will be added (e.g., john.doe1@company.com)
                        </Typography>
                    </Alert>
